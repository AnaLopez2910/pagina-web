...
alter table public.products add column if not exists purchase_price integer;
alter table public.order_items add column if not exists purchase_price integer;

do $$
begin
  if exists (select 1 from public.products where purchase_price is null) then
    raise exception 'No se puede hacer obligatorio purchase_price: hay productos sin precio de compra.';
  end if;

  if exists (select 1 from public.order_items where purchase_price is null) then
    raise exception 'No se puede hacer obligatorio purchase_price: hay líneas de pedidos sin precio de compra.';
  end if;

  execute 'alter table public.products alter column purchase_price set not null';
  execute 'alter table public.order_items alter column purchase_price set not null';

  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.products'::regclass
      and conname = 'products_purchase_price_check'
  ) then
    execute 'alter table public.products add constraint products_purchase_price_check check (purchase_price >= 0)';
  end if;
  ...
  if not exists (
    ...
    where conrelid = 'public.order_items'::regclass
      and conname = 'order_items_purchase_price_check'
  ) then
    execute 'alter table public.order_items add constraint order_items_purchase_price_check check (purchase_price >= 0)';
  end if;
end;
$$;
